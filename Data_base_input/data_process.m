...
% 遍历a0的值
for a0 = 0.1:0.1:10
    [Gm, xx, uu, du, dt, dudt, t] = fig1b(); % 修改此处以接收返回值
    % 创建和检查文件夹
    ...
    %ylim([-5 5]); zlim([-1 1]); xlim([0 250]);
    set(gca, 'Linewidth', 1, 'Fontsize', 18);
    view(-45, 10);
    image_name = fullfile(folder_name, sprintf('a0=%.1f.fig', a0));
    savefig(gcf, image_name);  % 使用 savefig 来保存 .fig 文件
    close(gcf);
end

function [Gm, xx, uu, du, dt, dudt, t] = fig1b()
    % 仿真函数，计算电子的轨迹和能量增益
    global a0 L b0 u0 zf del p0 z0 gm0
    yyi = [0 0 z0 0 0 u0 gm0];

    num_points = 2000;  % 你想要的固定步数
